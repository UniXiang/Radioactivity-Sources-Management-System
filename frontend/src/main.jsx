...
const statusText = { in_stock: '在库', checked_out: '借出', calibrating: '刻度中', acu_in_use: 'ACU在用', cls_in_use: 'CLS在用', inactive: '停用' }
...
function Dashboard({ navigate, onError }) {
  ...
  return <><section className="metric-grid">{[['total', '管理中 Source'], ['in_stock', '在库'], ['checked_out', '借出'], ['calibrating', '刻度中'], ['acu_in_use', 'ACU在用'], ['cls_in_use', 'CLS在用']].map(([key, label]) => <div className="metric" key={key}><span>{label}</span><strong>{data.counts[key]}</strong><small>当前 Registry</small></div>)}</section>
    <section className="dashboard-grid"><Panel title="当前借出"><SourceMiniList items={data.checked_out} empty="没有借出的 Source" navigate={navigate} /></Panel><Panel title="当前刻度中"><SourceMiniList items={data.calibrating} empty="没有正在刻度的 Source" navigate={navigate} /></Panel><Panel title="ACU在用"><SourceMiniList items={data.acu_in_use} empty="没有 ACU 在用的 Source" navigate={navigate} /></Panel><Panel title="CLS在用"><SourceMiniList items={data.cls_in_use} empty="没有 CLS 在用的 Source" navigate={navigate} /></Panel></section>
...
function SourceList({ navigate, onError, user }) {
  ...
  return <><div className="toolbar"><input placeholder="搜索 Source ID、名称、序列号" value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && load()} /><input placeholder="核素" value={isotope} onChange={e => setIsotope(e.target.value)} onKeyDown={e => e.key === 'Enter' && load()} /><input placeholder="位置" value={location} onChange={e => setLocation(e.target.value)} onKeyDown={e => e.key === 'Enter' && load()} /><select value={status} onChange={e => setStatus(e.target.value)}><option value="">全部状态</option><option value="in_stock">在库</option><option value="checked_out">借出</option><option value="calibrating">刻度中</option><option value="acu_in_use">ACU在用</option><option value="cls_in_use">CLS在用</option><option value="inactive">停用</option></select><label className="checkbox"><input type="checkbox" checked={inactive} onChange={e => setInactive(e.target.checked)} />显示停用</label>{user.role === 'admin' && <button className="primary compact" onClick={() => navigate('/sources/new')}>新建 Source</button>}<button className="secondary compact" onClick={() => download('/sources/export.csv')}>导出 CSV</button></div>
...
function SourceDetail({ id, user, navigate, onError }) {
  ...
  const stateActions = source.status === 'in_stock' ? <><button disabled={busy} onClick={() => setModal('checkout')}>借出</button><button disabled={busy} onClick={() => setModal('start')}>开始刻度</button><button disabled={busy} onClick={() => action('acu/start', {})}>标记 ACU在用</button><button disabled={busy} onClick={() => action('cls/start', {})}>标记 CLS在用</button></> : source.status === 'checked_out' ? <><button disabled={busy} onClick={() => setModal('return')}>归还</button><button disabled={busy} onClick={() => setModal('start')}>开始刻度</button></> : source.status === 'calibrating' ? <button disabled={busy} onClick={() => action('calibration/end')}>结束刻度</button> : source.status === 'acu_in_use' ? <button disabled={busy} onClick={() => action('acu/end', {})}>结束 ACU使用</button> : source.status === 'cls_in_use' ? <button disabled={busy} onClick={() => action('cls/end', {})}>结束 CLS使用</button> : null
